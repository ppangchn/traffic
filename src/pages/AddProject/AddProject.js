...
class AddProject extends Component {
	...
	componentDidMount() {
		try {
			if (this.props.id) {
				axios.get(`http://dev.pirsquare.net:3013/traffic-api/project/${this.props.id}`).then(res => {
					const { data } = res
					// let filteredPM = [];
					// let pm = [];
			...
			axios.get(`http://dev.pirsquare.net:3013/traffic-api/users/pm`).then(res => {
				const { data } = res
	...
	render() {
		...
		return (
			<Container>
				...
				<Modal size="5" isOpen={this.state.open} toggle={onClose} onExit={() => this.clear()}>
								...
								{this.state.color.map(c => {
									return (
										<Col className="pd5" md={1} sm={1} xs={2}>
											<ColorButton color={c} setCheckedColor={this.setCheckColor} checkedColor={this.state.checkedcolor} />
										</Col>
									)
								})}
							</Row>
