		...
		try {
			if (this.state.name && this.state.roles && this.state.email) {
				...
				if (this.state.sendResetPass) {
					try {
						axios.post(`${url}/users/forgotpass`, data).then(console.log('send resetpassword!'))
					} catch (error) {
							...
							<Form onSubmit={this.sendDataMember}>
								...
								{['DEV', 'DSN', 'QA', 'TS'].indexOf(this.state.roles.label) === -1 && (
									<div className="checkbox" style={{ display: 'flex' }}>
										<input name="sendResetPass" type="checkbox" className="mr-3" onChange={this.handleCheckboxChange} id="resetpassword" />
										<label for="resetpassword" />
										<div className="sendresetpasswordtext">Send Reset Password</div>
									</div>
								)}
								<Row className="title">
									<Col className="mb-3">
